...
/**
 * The arrow used on every "open this" button in the design.
 *
 * @return string
 */
function techrato_arrow_icon() {
	return '<svg class="icon icon-arrow" viewBox="0 0 24 24" aria-hidden="true"><path d="M7 17L17 7M9 7h8v8"></path></svg>';
}

/**
 * A post's featured image at the given size, falling back to the placeholder so
 * a card never collapses to a hole in the grid.
 *
 * @param string $size Registered image size.
 * @return string
 */
function techrato_thumb( $size = 'techrato-card' ) {
	if ( has_post_thumbnail() ) {
		return get_the_post_thumbnail( null, $size, array( 'loading' => 'lazy', 'alt' => '' ) );
	}

	return '<img src="' . esc_url( TECHRATO_URI . '/assets/images/placeholder.svg' ) . '" alt="">';
}

/**
 * The first category of the current post, for the little label on each card.
 *
 * @return string
 */
function techrato_primary_category() {
	$cats = get_the_category();

	return isset( $cats[0] ) ? $cats[0]->name : '';
}

/**
 * Section heading shared by every block on the homepage.
 *
 * @param string $eyebrow Small label above the title.
 * @param string $title   Section title.
 * @param string $url     Optional "more" link.
 * @param string $more    Optional "more" label.
 * @param string $extra   Extra class on the wrapper.
 */
function techrato_section_heading( $eyebrow, $title, $url = '', $more = '', $extra = '' ) {
	?>
	<div class="section-heading<?php echo $extra ? ' ' . esc_attr( $extra ) : ''; ?>">
		<div>
			<?php if ( $eyebrow ) : ?>
		...
		<?php if ( $url ) : ?>
			<a class="more-button" href="<?php echo esc_url( $url ); ?>">
				<?php echo esc_html( $more ? $more : __( 'مشاهده بیشتر', 'techrato' ) ); ?>
				<span><?php echo techrato_arrow_icon(); // phpcs:ignore WordPress.Security.EscapeOutput ?></span>
			</a>
		<?php endif; ?>
	</div>
	<?php
