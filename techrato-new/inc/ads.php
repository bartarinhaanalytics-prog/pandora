...
/**
 * A native ad shaped like the surrounding cards.
 *
 * It borrows the listing's own card markup on purpose: a native ad that does
 * not sit in the rhythm of the grid is just a banner in a strange place. The
 * label and the button are what tell the reader it is an ad.
 *
 * @param array  $item Ad row.
 * @param string $card Card slug of the list it is being dropped into.
 */
function techrato_ads_native_row( $item, $card = 'news' ) {
	$url    = isset( $item['url'] ) ? $item['url'] : '';
	$button = '' !== trim( (string) $item['button'] ) ? $item['button'] : __( 'مشاهده', 'techrato' );
	$rel    = techrato_ads_rel( $item );
	$tag    = techrato_ads_tag();

	if ( 'category-post' === $card ) {
		$image = techrato_ads_image( $item, 'techrato-card' );
		?>
		<article class="category-post card-ad">
			<?php if ( $url ) : ?>
				<a class="category-post__image" href="<?php echo esc_url( $url ); ?>" rel="<?php echo esc_attr( $rel ); ?>" target="_blank"><?php echo $image; // phpcs:ignore WordPress.Security.EscapeOutput ?></a>
			<?php else : ?>
				<span class="category-post__image"><?php echo $image; // phpcs:ignore WordPress.Security.EscapeOutput ?></span>
			<?php endif; ?>
	...
	// Default: the four-across card used by "آخرین اخبار".
	$image = techrato_ads_image( $item, 'techrato-card' );
	?>
	<article class="news-card card-ad">
		<div class="news-card__image">
			<?php echo $image; // phpcs:ignore WordPress.Security.EscapeOutput ?>
			<?php if ( $url ) : ?>
				<a href="<?php echo esc_url( $url ); ?>" rel="<?php echo esc_attr( $rel ); ?>" target="_blank" aria-label="<?php echo esc_attr( $item['title'] ); ?>"><?php echo techrato_arrow_icon(); // phpcs:ignore WordPress.Security.EscapeOutput ?></a>
			<?php endif; ?>
		</div>
		<?php if ( $tag ) : ?>
			<span class="ad-tag ad-tag--inline"><?php echo esc_html( $tag ); ?></span>
